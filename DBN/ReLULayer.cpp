...
void Activator::activateReLULayer(ReLULayer* s, Layer* layer, Up_flag_t up){
   ...
   for (int i = 0; i < s->nodenum_; ++i){
      for (int j = 0; j < s->batchsize_; ++j){
         float sigprob = softplus(gsl_matrix_float_get(s->preactivations_, i, j));
...
void ReLULayer::activate(Activator& act, Layer* layer, Up_flag_t UFLAG){
   act.activateReLULayer(this, layer, UFLAG);
}
